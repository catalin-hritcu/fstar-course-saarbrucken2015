...
#set-options "--__temp_no_proj WeakestPrecondition"
...
| Repeat  : cond:bexp -> body:com -> inv:pred -> com
...
(* Evaluation relation *)
noeq type reval : com -> heap -> heap -> Type =
| ESkip      : h0:heap ->
               reval Skip h0 h0
...
| ERepeatEnd  :h0:heap -> h1:heap -> be:bexp{eval_bexp h1 be = true} ->
               cb:com -> i:pred -> 
               reval cb h0 h1 -> 
               reval (Repeat be cb i) h0 h1

| ERepeatLoop :h0:heap -> h1:heap -> be:bexp{eval_bexp h1 be = false} ->
               cb:com -> i:pred -> h2:heap ->
               reval cb h0 h1 ->
               reval (Repeat be cb i) h1 h2 ->
               reval (Repeat be cb i) h0 h2
...
let rec eval c h0 =
  ...
  | Seq c1 c2 ->
     ...
     (|h2, ESeq h0 c1 c2 h1 h2 p1 p2|)
  | If be ct ce ->
     if eval_bexp h0 be then
       let (|h1, p1|) = eval ct h0 in
       (|h1, EIfTrue h0 be ct ce h1 p1|)
     else
       let (|h1, p1|) = eval ce h0 in
       (|h1, EIfFalse h0 be ct ce h1 p1|)
  | While be cb i ->
     if eval_bexp h0 be
     then let (|h1, p1|) = eval cb h0 in
          let (|h2, p2|) = eval c  h1 in
          (|h2, EWhileLoop h0 be cb i h1 h2 p1 p2|)
     else (|h0, EWhileEnd h0 be cb i|)
  | Repeat be cb i ->
     let (|h1, p1|) = eval cb h0 in
       if eval_bexp h1 be
       then (|h1, ERepeatEnd h0 h1 be cb i p1|)
       else let (|h2, p2|) = eval c h1 in
            (|h2, ERepeatLoop h0 h1 be cb i h2 p1 p2|)
...
type hoare (p:pred) (c:com) (q:pred) : Type0 =
  (forall h h'. squash (reval c h h') ==> valid (p h) ==> valid (q h'))
...
let bpred be h = FEq (eval_bexp h be) true
