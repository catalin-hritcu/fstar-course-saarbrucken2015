...
let rec progress he _ _ h =
  match h with
  | TyApp #g #e1 #e2 #t11 #t12 h1 h2 ->
     (match e1, e2 with
      | ELam t e1', _ -> ExIntro (Cfg he (subst_beta e2 e1')) (SBeta t he e1' e2)
      | ECon CRead, ECon (CRef r) ->
         ExIntro (Cfg he (ECon (CInt (he r)))) (SRead he r)
      | EApp (ECon CWrite) (ECon (CRef r)), ECon (CInt i) ->
         ExIntro (Cfg (heap_upd he r i) (ECon CUnit)) (SWrite he r i)
      | _ ->
         (* Why are the implicits passed to SAppX needed? *)
         if is_value e1 then
          (let ExIntro c h2' = progress he e2 h2 in
           let Cfg he' e2' = c in
           ExIntro (Cfg he' (EApp e1 e2')) (SApp2 #he e1 #he' #e2 #e2' h2'))
         else
           let ExIntro c h1' = progress he e1 h1 in
           let Cfg he' e1' = c in
           ExIntro (Cfg he' (EApp e1' e2)) (SApp1 #he #e1 #he' e2 #e1' h1')
     )
...
let rec preservation c c' hs g t ht =
  let TyApp h1 h2 = ht in
    match hs with
    | SBeta t h e1' e2' -> (let TyLam t_x hbody = h1 in
    ...
    | SRead h r   -> let ECon (CInt i) = Cfg.e c' in TyCon (CInt i)
    | SWrite h r i-> TyCon CUnit
