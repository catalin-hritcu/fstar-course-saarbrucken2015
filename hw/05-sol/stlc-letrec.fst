  ...
  | EApp e1 e2 ->
      if is_value e1 then
        if is_value e2 then
          ...
          | ERec f tf x tx body -> Some (subst x e2 (subst f e1 body))
          | _           -> None
        else
          match (step e2) with
...
let rec typing g e =
  ...
  | EAbs x t e1 ->
      ...
      | None    -> None)
  | ERec f tf x tx e -> (match typing (extend (extend g x tx) f tf) e with
...
let rec free_in_context x e g =
  ...
  | ERec f tf y ty body -> free_in_context x body (extend (extend g y ty) f tf)
...
let rec context_invariance e g g' =
  ...
  | ERec f tf x tx body ->
     context_invariance body (extend (extend g x tx) f tf) (extend (extend g' x tx) f tf)
...
let rec substitution_preserves_typing x e v g =
  ...
  | ERec f tf y ty body -> 
     let gxyf = extend (extend gx y ty) f tf in
     let gyf = extend (extend g y ty) f tf in
  ...
  | EApp e1 e2 ->
     ...
     then (if is_value e2
           then match e1 with 
                ...
                | ERec f tf x tx body -> (substitution_preserves_typing f body e1 (extend empty x tx);
                                          substitution_preserves_typing x (subst f e1 body) e2 empty)
