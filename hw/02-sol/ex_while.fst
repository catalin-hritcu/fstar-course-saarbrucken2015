...
let rec eval h s =
  ...
  | Cond ec st se  ->
    (match eval_exp h ec with
    | VBool b -> eval h (if b then st else se)
    | _ -> raise TypeError)
...
let rec typecheck g s =
  ...
  | Cond c t e ->
     (match typecheck_exp g c, typecheck g t, typecheck g e with
...
(* evaluation function for well typed stmts (can't diverge without While) *)
val eval_div : g:env ->
               h:heap{typed_heap g h} -> 
               s:stmt{typecheck g s} -> 
               Tot (h':heap{typed_heap g h'}) (decreases s)
let rec eval_div g h s =
  match s with
  | Skip -> h
  | Assign x t -> update h x (eval_exp_tot g h t)
  | Seq s1 s2 -> eval_div g (eval_div g h s1) s2 
  | Cond c t e  -> let VBool b = eval_exp_tot g h c in
                   eval_div g h (if b then t else e)
